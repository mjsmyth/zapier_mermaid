...
Input exports live in dated folders at the repo root. The JSON filename **must match** the folder name exactly:
```
zapfile_20260526_01/
    zapfile_20260526_01.json
```

Pass the folder path to the script; it derives the JSON path automatically. Output is written to `output/` at the repo root as `zapier-<title-slug>.md`. Zaps with `"status": "off"` are skipped; the slug is the lowercased, hyphenated Zap title. Warnings and errors are logged to `zapier_mermaid.log`.
