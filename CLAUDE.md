...
``` Output is written to `output/` at the repo root as `zapier-<title-slug>.md`. Zaps with `"status": "off"` are skipped; the slug is the lowercased, hyphenated Zap title. Warnings and errors are logged to `zapier_mermaid.log`.
...
**YAML style values:** Two providers currently have a spurious leading word in their `style` value — `Google Drive` (`yellow fill:#FEFBD0,...`) and `Google Sheets` (`pink fill:#FBEAF0,...`). These are editorial notes left in the value; they produce invalid Mermaid CSS. The style string must start with a valid CSS property like `fill:`.
